...
<body>
	...
	<div class="container">
		<div class="p-3 mt-5 rounded-3 shadow" >
			<h3 class="fw-bold">Data Album</h3>
			<div class="box">
				<p><a href="tambah-album.php" class="btn btn-success"><i class="bi bi-plus"></i>Tambah Data</a></p>
				<table border="1" cellspacing="0" class="table table-striped table-bordered">
					...
					<tbody>
						...
						$foto = mysqli_query($conn, "SELECT * FROM album
							WHERE user_id = '$user' "); if (mysqli_num_rows($foto) > 0) {
								while ($row = mysqli_fetch_array($foto)) { ?>
									<tr>
										...
										<td> 
											<a class="btn btn-danger btn-sm" href="proses-hapus-album.php?idp=<?php echo
											$row['album_id'] ?>" onclick="return confirm('Yakin Ingin Hapus ?')"><i style="font-style: normal;" class="bi bi-trash"> Hapus</i></a>
										</td>
									</tr>
								<?php }
							} else { ?>
